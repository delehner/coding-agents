...
| `src/pipeline/agent.rs` | Ralph Loop; provider CLI with null stdin (headless); JSONL tail + hints on non-zero exit; `devcontainer exec` when Dev Container enabled | Changing iteration logic |
